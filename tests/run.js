...
const tests = [
  ...
  },
  {
    ...
    run() {
      ...
      assert.equal(result.classification.category, Category.PERSONAL);
      assert.ok(result.classification.confidence >= 70);
    }
  },
  {
    name: "raises confidence for Gmail-labeled personal mailbox threads",
    run() {
      const result = evaluateEmail({
        subject: "move list",
        plainTextBody: "These are just a few things i want to talk about before moving.",
  ...
  {
    name: "raises confidence for sent personal mailbox threads",
    run() {
      const result = evaluateEmail({
        subject: "New logo generated by ChatGPT",
        plainTextBody: "I sent an invite and wanted to share the update.",
        senderDomain: "gmail.com",
        labels: ["SENT"]
      });
